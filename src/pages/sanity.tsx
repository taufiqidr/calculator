...
const Sanity: NextPage = () => {
  const [sanity, setSanity] = useState(0);
  const [CurrSanity, setCurrSanity] = useState(0);
  const [time, setTime] = useState(new Date());

  useEffect(() => {
    setSanity(135 - CurrSanity);
  }, [CurrSanity]);

  return (
    <>
      <Head>
        <title>Sanity Calculator</title>
        ...
        <div className="mx-4 flex flex-col gap-y-4">
          ...
          <input
            ...
            onChange={(e) => {
              if (Number(e.target.value) < 0) {
                ...
                setCurrSanity(Number(e.target.value));
              }
            }}
          />
          <div className="flex items-center ">
            <button
              ...
              onClick={() => {
                const customTimeMs = sanity * 6 * 60 * 1000; // convert minutes to milliseconds
          ...
          <div className=" text-center tracking-tight text-white sm:text-[5rem] ">
            Will be full at:
            {CurrSanity == 0 ? "" : ` ${time.toLocaleTimeString()}`}
          </div>
        </div>
      </main>
    </>
